...
export async function sendInvitationEmail(
  ...
  const setupUrl = `${baseUrl}/konto-einrichten?token=${token}`;
    ...
    html: `
      <div style="font-family: 'Inter', Arial, sans-serif; max-width: 600px; margin: 0 auto;">
        ...
        <div style="background: white; padding: 32px; border: 1px solid #e2e8f0; border-top: none; border-radius: 0 0 12px 12px;">
          ...
          <p style="color: #475569;">du wurdest zum <strong>Hellbeck HR Portal</strong> eingeladen. Klicke auf den folgenden Button, um dein Konto einzurichten und ein Passwort zu setzen:</p>
          <div style="text-align: center; margin: 24px 0;">
            <a href="${setupUrl}" style="display: inline-block; background: #4c6ef5; color: white; padding: 12px 32px; border-radius: 8px; text-decoration: none; font-weight: 600; font-size: 16px;">Konto einrichten</a>
          </div>
          <p style="color: #94a3b8; font-size: 14px;">Dieser Link ist <strong>7 Tage</strong> gültig. Falls du diese E-Mail nicht erwartet hast, kannst du sie ignorieren.</p>
          <p style="color: #94a3b8; font-size: 12px; margin-top: 8px;">Falls der Button nicht funktioniert, kopiere diesen Link in deinen Browser:<br/><a href="${setupUrl}" style="color: #4c6ef5; word-break: break-all;">${setupUrl}</a></p>
